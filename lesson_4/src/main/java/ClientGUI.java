...
public class ClientGUI extends JFrame implements ActionListener, Thread.UncaughtExceptionHandler {
    ...
    ClientGUI(){
        ...
        listUsers.setListData(new String[]{"user1", "user2", "user3", "user4","user5", "user6", "user7",
        "user8", "user9", "user0"});
    ...
    public void sendMessage(){
        ...
        if(text.length() > 1) {
            ...
            printToFileMessage(text);
        }

    }

    public static void printToFileMessage(String text){
        try(BufferedWriter out = new BufferedWriter
                (new FileWriter("messageHistory.txt",true))){
            out.write(text );
        }catch (IOException e){
            e.printStackTrace();
        }
